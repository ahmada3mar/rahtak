<template>
    <div class="skeleton-body">
        <div class="skel-pro-single" :class="{loaded: loaded}">
            ...
            <div class="skel-widget"></div>
            <div
                :class="toggleState ? 'sidebar-filter right sidebar-product' : 'sidebar sidebar-product'"
            >
                <button title="Close (Esc)" type="button" class="mfp-close" @click="toggleSidebar">
                    <span>×</span>
                </button>
                <related-products-two :products="relatedProducts"></related-products-two>

                <div class="widget widget-banner-sidebar">
                    <div class="banner-sidebar-title">ad box 280 x 280</div>

                    <div class="banner-sidebar banner-overlay">
                        <nuxt-link :to='localePath("/shop/sidebar/4cols")'>
                            <img v-lazy="'./images/blog/sidebar/banner.jpg'" alt="banner" />
                        </nuxt-link>
                    </div>
                </div>
            </div>
            <button class="sidebar-fixed-toggler right" @click="toggleSidebar" v-if="toggleState">
                <i class="icon-cog"></i>
            </button>
